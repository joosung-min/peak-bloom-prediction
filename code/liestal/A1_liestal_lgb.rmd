...
final_pred_df <- data.frame(city = "Liestal", method = "ML", bloom_doy = final_bloom_doy - ceiling(min(MAE_p)))
final_pred_probs <- data.frame(city = "Liestal", date = weather_2023[, "date"], pred_probs = final_pred)
```

```{r eval=FALSE, echo=FALSE}
write.csv(final_pred_df, "/workspaces/peak-bloom-prediction/code/liestal/data/A19_final_lgb_predDay_liesetal.csv", row.names = FALSE)
...
* Therefore, the final prediction date for 2023 from this model is `r {as.Date(final_pred_day1) - ceiling(min(MAE_p)/2)}`.
