...
cherry_df <- F01_train_val_test_split(gdd_df = cherry_gdd, val_year = c(2019, 2020), train_year = c(2021, 2022), n_folds = 8)
...
#     max_depth <- as.numeric(param_grid[["max_depth"]])
    ...
    cv_table <- matrix(nrow = n_fold, ncol = 2
        , dimnames = list(NULL, c("fold", "auc")))
    ...
    for (f in 1:n_fold) {
        ...
        cv_table[f, ] <- c(f, lgb_val$best_score)

    }

    avg_score <- mean(cv_table[, 2], na.rm = TRUE)
    out <- c(boosting, learning_rate, max_bin, min_data_in_leaf, num_leaves, max_depth, avg_score)
    # cv_result[1, ] <- out
    return(out)
}
...
print(best_auc)
write.csv(best_auc, grid_best_filename, row.names = FALSE)
...
# val_cv <- read.csv("./outputs/A14_Liestal_test.csv")
