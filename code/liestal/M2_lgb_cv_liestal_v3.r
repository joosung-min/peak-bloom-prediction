...
n_fold <- 8
cherry_df <- F01_train_val_test_split(gdd_df = cherry_gdd, val_year = c(2019, 2020), test_year = c(2021, 2022), n_fold = 8)

feature_names <- c("month", "day", "Cd_cumsum", "Ca_cumsum", "lat", "long", "alt", "daily_Ca", "daily_Cd", "tmax", "tmin")

target_col <- "is_bloom"

Rdata_name <- "./outputs/M21_lgb_RDada_Liestal3.RData"
grid_result_filename <- "./outputs/M22_lgb_grid_Liestal3.csv"
grid_best_filename <- "./outputs/M23_lgb_best_params_Liestal3.csv"
...
# # for (g in seq_len(nrow(grid_search))) {
    ...
    cv_table <- matrix(nrow = n_fold, ncol = 3
        , dimnames = list(NULL, c("fold", "val_auc", "test_auc")))
