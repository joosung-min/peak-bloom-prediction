...
Rdata_name <- "./code/kyoto/outputs/M21_lgb_RDada_kyoto4.RData"
grid_result_filename <- "./code/kyoto/outputs/M22_lgb_grid_kyoto4.csv"
grid_best_filename <- "./code/kyoto/outputs/M23_lgb_best_score_kyoto4.csv"
lgb_final_name <- "./code/kyoto/outputs/M24_lgb_final_kyoto4.rds"
...
grid_search <- expand.grid(
    ...
    , max_depth = c(-1, 10, 20)
    , feature_fractions = c(1, 0.9, 0.8)
    , bagging_fractions = c(1, 0.9, 0.8)
...
for (g in seq_len(nrow(grid_search))) {
    ...
    cv_table <- matrix(nrow = n_fold, ncol = 2
        , dimnames = list(NULL, c("fold", "val_score")))
