...
for (p_thresh in target_p_thresh) {
    ...
    for (yr in MAE_table$year) {
        ...
        actual_bloom_date <- read.csv("/workspaces/peak-bloom-prediction/competition_rules/data/kyoto.csv") %>%filter(year == yr) %>% pull(bloom_date)
        mae_set <- read.csv("/workspaces/peak-bloom-prediction/code/kyoto/data/A14_kyoto_gdd.csv") %>% filter(year == yr)
