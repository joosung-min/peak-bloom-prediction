## Cross-validation using Focal loss function
...
grid_search <- expand.grid(boostings = c("dart", "gbdt")
    ...
    mutate(iteration = NA) %>%
    mutate(focal_loss = NA)
    
# loss_functions <- c("focal_loss", "auc", "binary_error")
# loss_functions <- c("focal_loss", "focal_loss", "auc")
...
grid_search_result <- foreach (
    ...
    metricDF <- data.frame(
        iteration = seq_len(length(cv_metrics$focal_loss$eval))
        , focal_loss = round(unlist(cv_metrics[["focal_loss"]][["eval"]]), 3)
...
# save(grid_search_result, file = "../outputs/B_outputs/B11_lgb_grid_kyoto_focal.RData")
...
write.csv(grid_search_out, "../outputs/B_outputs/B11_lgb_grid_kyoto_focal.csv", row.names = FALSE)
...
grid_search_out <- read.csv("../outputs/B_outputs/B11_lgb_grid_kyoto_focal.csv")
head(grid_search_out)


best_loss <- grid_search_out[which(grid_search_out$focal_loss == min(grid_search_out$focal_loss)), ]
write.csv(best_params, "../outputs/B_outputs/B11_lgb_grid_kyoto_best_params_focal.csv", row.names = FALSE)

# Fit final model:
# params
param_idx <- 1 # best focal_loss
...
saveRDS.lgb.Booster(lgb_final, file = "../outputs/B_outputs/B21_lgb_focal.rds")
